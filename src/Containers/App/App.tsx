...
import { ComponentProps, Editor, EditorRef, Playground } from "../../Components";
import { appendComponent, updateComponentProps, removeComponent, findOrCreateExpression, getPositionByIndex, getTypeFromIndex, getSlotFromIndex, removeExpresiionFromComponent } from "../../Core";
...
export const App = () => {
    ...
    const playgroundChangeHandler = useCallback((index: number, type: string, name: string, slot: string) =>
        setCode(code => appendComponent(code, index, type, name, slot)), []);

    const playgroundRemoveHandler = useCallback((index: number, slot: string) =>
        setCode(code => removeComponent(code, index, slot)), []);

    const playgroundSelectHandler = useCallback((index: number, type: string, slot: string) => {
        if (refEditor.current) {
            refEditor.current.setPosition(...getPositionByIndex(code, index));
            refEditor.current.focus();
        }
        setSelected(curSelected => curSelected?.index === index ? curSelected : { index, slot, type });
    }, [code]);

    const cursorChangeHandler = useCallback((index: number) => {
        setSelected(curSelected => curSelected?.index === index ? curSelected : { index, type: getTypeFromIndex(code, index), slot: getSlotFromIndex(code, index) });
    }, [code])

    const propertyChangeHandler = useCallback((index: number, key: string, type: any, value: any) => {
        setCode(code => updateComponentProps(code, index, key, type, value));
    }, [])

    const createExpressionHandler = useCallback((index: number, key: string, template?: any) => {
        const [newCode, isNew] = findOrCreateExpression(code, index, key, template, 'Handler');
        if (isNew) {
            setCode(newCode);
        }
    }, [code]);

    const removeExpressionHandler = useCallback((index: number, slot: string) =>
