...
const getRect = (element: Element) => {
    ...
    for (const child of children) {
        const childRect = child.style.display === 'contents'
            ? getRect(child)
            : child.getBoundingClientRect();
        newRect.left = childRect.left <= newRect.left ? childRect.left : newRect.left;
        newRect.right = childRect.right >= newRect.right ? childRect.right : newRect.right;
        newRect.top = childRect.top <= newRect.top ? childRect.top : newRect.top;
        newRect.bottom = childRect.bottom >= newRect.bottom ? childRect.bottom : newRect.bottom;
    }
...
export const Control: React.FC<Props> = ({ elemRef, ...dataset }) => {
    const { onChange, onSelect, onRemoveComponent, selected } = useContext(FrameContext);
    ...
    //@ts-ignore
    const isSelected = selected?.index == dataset?.['data-builder-index'] &&
    ...
    const children = Array.isArray(scd?.children) ? sc.filter(([name]) => (scd?.children as string[]).includes(name)) : sc;
    ...
    const element = (
        <div
            ...
            {showContextMenu && <DropList fixed coords={contextMenuPos}>
                <DropList.Item
