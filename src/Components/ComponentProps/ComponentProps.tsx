...
export const ComponentProps: React.FC<Props> = ({ index, code, onChange, onExpression, onRemoveExpression }) => {
    if (index !== undefined) {
            ...
            <Groups design='vertical' height='auto' alignItems='center'>
                {
                    Object
                        ...
                        .map(([key, data]) => {
                            ...
                            const disabled = deps.length > 0 ? !deps.some(([propName, values]) => values.includes(componentProps[propName].value)) : false;
