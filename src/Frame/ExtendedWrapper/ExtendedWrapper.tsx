...
export const ExtendedWrapper: React.FC = ({ children, ...attrs }: any) => {
        ...
        Object.entries(componentData.props)
            ...
            .forEach(([name, data]) => {
                if (name === 'children' && allowChildren !== undefined) {
                    // Wrap child ***
                    replacedProps[name] = allowChildren ? replacedProps[name] ?? placeholder : undefined;
                    return;
                }

                if (replacedProps[name]) {
                    replacedProps[name] = <ExtendedWrapper><>{replacedProps[name]}</></ExtendedWrapper>;
                }
                else {
                    replacedProps[name] = <ExtendedWrapper
    ...
    return (
        <>
            <div ref={ref} style={{ display: 'contents' }}>{wrappedChildren}</div>
