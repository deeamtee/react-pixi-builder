...
export const appendComponent = (code: string, idx: number, type: string, newElementName: string, slot: string): string => {
    try {
            ...
            visitJSXOpeningElement(element) {
                if (index === idx && print(element.value.name).code === type) {
                    ...
                    if (slot === 'undefined') {
                        ...
                        const template = (identifier: string) => `const ${identifier} = ${prettyPrint(newElement)}`;
                        const [newCode, isNew] = findOrCreateExpression(
                            code, idx, slot, template
                        );
